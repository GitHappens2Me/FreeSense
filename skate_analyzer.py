#!/usr/bin/env python3
"""
Ice Skate Motion Analysis & 3D Trajectory Visualization
=======================================================

Analyzes IMU data with pre-calculated orientation (pitch/yaw/roll) to 
reconstruct 3D skating path. Includes drift correction and visualization.

Usage:
    python skate_analyzer.py skate_data.csv
...
class SkateConfig:
    ...
    # Filtering
    acc_cutoff_hz: float = 5.0      # Lowpass filter for acceleration
    velocity_cutoff_hz: float = 2.0 # Highpass filter for velocity drift
    
    # Drift correction
    zero_velocity_threshold: float = 0.3  # m/s - threshold to detect stationary
    min_stationary_duration: float = 0.5  # seconds
...
class OrientationUtils:
    ...
    def euler_to_rotation_matrix(pitch: float, yaw: float, roll: float, degrees: bool = True) -> np.ndarray:
        ...
        # Combined: R = Rz * Ry * Rx (applied in order: roll, pitch, yaw)
        return Rz @ Ry @ Rx
...
class SkateAnalyzer:
    """Main analysis class for skate IMU data."""
    ...
    def load_data(self, filepath: str) -> pd.DataFrame:
        ...
        if df['timestamp'].iloc[-1] > 1e9:  # Microseconds
            df['time_sec'] = (df['timestamp'] - df['timestamp'].iloc[0]) / 1e6
        elif df['timestamp'].iloc[-1] > 1e6:  # Milliseconds
            df['time_sec'] = (df['timestamp'] - df['timestamp'].iloc[0]) / 1e3
        else:  # Already seconds
            df['time_sec'] = df['timestamp'] - df['timestamp'].iloc[0]
        
        self.df = df
        self.timestamps = df['time_sec'].values
        self.dt = np.mean(np.diff(self.timestamps))
    ...
    def highpass_filter(self, data: np.ndarray, cutoff: float) -> np.ndarray:
        """Apply Butterworth highpass filter for drift removal."""
        nyquist = 1 / (2 * self.dt)
        normal_cutoff = cutoff / nyquist
        if normal_cutoff >= 1.0:
            return data
    ...
    def process_accelerations(self) -> np.ndarray:
        ...
        # Remove gravity (z-component should be ~g when stationary)
        # Use median of z as gravity estimate
    ...
    def integrate_to_velocity(self) -> np.ndarray:
        ...
        for i in range(3):
            velocity[:, i] = self.highpass_filter(velocity[:, i], self.config.velocity_cutoff_hz)
    ...
    def detect_stationary_periods(self) -> np.ndarray:
        """
        Detect when skate is stationary (for drift correction).
        Uses acceleration magnitude and velocity magnitude.
        """
        ...
        # Stationary when both are low
        stationary = (acc_mag < self.config.zero_velocity_threshold) & \
                     (vel_mag < self.config.zero_velocity_threshold)
        
        # Require minimum duration
        from scipy.ndimage import binary_dilation, binary_erosion
        stationary = binary_erosion(binary_dilation(stationary, iterations=5), iterations=5)
    ...
    def apply_zero_velocity_update(self) -> np.ndarray:
        ...
        # When stationary, force velocity to zero
...
class Visualizer:
    """Create visualizations for the skate data."""
    ...
    def plot_3d_trajectory(self, save_path: Optional[str] = None):
        ...
        ax.set_title('Ice Skate 3D Trajectory\n(colored by velocity)', fontsize=13)
    ...
    def plot_analysis_dashboard(self, save_path: Optional[str] = None):
        ...
        scatter = ax2.scatter(pos[:, 0], pos[:, 1], c=vel_mag, cmap='viridis', s=10)
        ...
        plt.suptitle('Ice Skate Motion Analysis Dashboard', fontsize=14, fontweight='bold')
        plt.tight_layout()
    ...
    def print_statistics(self):
        ...
        print("\n" + "="*50)
        print("SKATING SESSION STATISTICS")
        print("="*50)
...
def main():
    """Main entry point."""
    if len(sys.argv) < 2:
        print("Usage: python skate_analyzer.py <csv_file>")
    ...
    try:
        ...
        viz.plot_3d_trajectory(save_path="skate_3d_trajectory.png")
        viz.plot_analysis_dashboard(save_path="skate_analysis_dashboard.png")
        ...
        output_df.to_csv("skate_processed_data.csv", index=False)
        print("\nSaved processed data to skate_processed_data.csv")
